...
class UserReportController extends AbstractController
{
    ...
    public function getEmployeeAverageWorkTime(Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        if ($userId && $teamId) {
            return new JsonResponse([
                'error' => 'Cannot provide both user_id and team_id values in the query string. Please disable one of them.',
            ], 400);
        }

        if (!$userId && !$teamId) {
            return new JsonResponse([
                'error' => 'Please provide a value for either user_id or team_id in the query string.',
            ], 400);
        }
        ...
        if ($type === 'user') {
            ...
            foreach ($clocks as $clock) {
                ...
                switch ($period) {
                    case 'week': $key = $ts->format('o-W'); break;
                    case 'month': $key = $ts->format('Y-m'); break;
                    case 'year': $key = $ts->format('Y'); break;
                    default: $key = $ts->format('Y-m-d');
                }
                $groupedRecords[$key][] = ['type'=>$clock->getType(),'timestamp'=>$ts];
            }

            $periodSeconds = array_map(fn($records) => $this->calculateTotalSeconds($records), $groupedRecords);
...
//            $teamId = $request->query->get('team_id'); // new param
            ...
            // Fetch clocks in period **only for members of this team**
            $clocks = $em->createQueryBuilder()
                ->select('c')
                ->from(Clock::class, 'c')
                ->join('c.teamMember', 'tm')
            ...
            // Build team data for this single team
            $teamData = $this->buildTeamData($em, $memberSeconds = $this->computeMemberWorkSeconds($memberClocks, $period), $memberTeam, $period);

            // Keep only the requested team
            $teamData = array_filter($teamData, fn($t) => $t['id'] == $teamId);
        ...
        // At the very end, after the if/elseif block
